...
- `Repository`：通过 QSQLITE 实现用户、管理员、站点、电桩和订单的数据访问，以及订单事务读写；连接后校验外键、结构版本、五张核心表和两张管理员附属表；
...
管理员演示账号是 `admin / 123456`。首次成功登录旧数据库时，服务端会把该账号的旧 SHA-256 摘要自动升级为带随机盐的 PBKDF2-SHA256。新建管理员首次登录后必须修改初始密码，修改成功后需重新登录。在 Qt Creator 中先运行 CMake，然后将运行目标从
...
系统管理员可进入“管理员管理”页面，创建账号、修改显示名称、分配角色和站点范围、启用或停用账号。站点管理员只查看和操作授权站点；用户管理员可查看用户与订单并冻结或解冻用户。所有管理员都可从左下角修改自己的密码，修改后当前会话失效并返回登录页。
