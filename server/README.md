...
扫一扫报修复用工单页：报修记录带有“报修”标识、桩编号与故障类型，系统管理员可回复、
设为处理中或已解决，用户刷新“我的工单”查看结果。报修不自动改变桩或订单状态。
SQLite 报修需[迁移 004](../database/migrations/004_repair_tickets.sql)，新服务端兼容 schema 1–4；
schema 2/3 继续支持普通客服工单，权限仍仅限系统管理员。
...
- `Repository`：通过 QSQLITE 访问用户、管理员、站点、电桩、订单和工单；按 schema 1/2/3
  校验基线、工单及管理员扩展结构，未知或同版本异构库明确拒绝；
- `InMemoryRepository`：保留为显式启用的开发和单元测试替身，进程退出后修改丢失；
- 管理员 UI：支持登录、7/30 日及自定义日期概览、站点搜索/筛选/新增/安全删除和站内电桩展开（新增站点时可编辑初始电桩编号、类型和功率）、电桩搜索/筛选/新增/安全删除/上线/下线/重启/故障标记、用户查询及冻结/解冻、订单查看，以及管理员账号创建、编辑、停用、角色和站点范围授权；
- `AdminFacade`：所有管理员界面操作均通过 `ApplicationService`，UI 不直接访问 Repository；
- `charging_server_tests`：覆盖基础 TCP 业务、管理员登录、Dashboard、自定义日期、站点新增/安全删除、电桩生命周期、冻结限制和管理员 RBAC；
...
单价四舍五入后沿用原金额公式；跨时段、自动结束、重启及补付只使用快照。
不新增迁移，不改旧账单，不做谷价、快慢充倍率或管理配置；本策略不是当地真实电价。
...
管理员演示账号是 `admin / 123456`。完成 003 管理员迁移后，首次成功登录会把旧 SHA-256
摘要升级为带随机盐的 PBKDF2-SHA256；未升级的 schema 1/2 仍可固定账号登录，不自动改写密码。
新建管理员首次登录后必须修改初始密码，修改成功后需重新登录。在 Qt Creator 中先运行 CMake，然后将运行目标从
...
登录页采用左侧悦充品牌、右侧账号密码布局；蓝色侧栏按四类业务分组。
按钮、字体、表格和客服工单统一视觉。运营监控包含 4 张图，营收统计包含 6 张图，
涵盖设备状态、站点占用/异常、订单状态、实收、支付单数、电量、站点排名、模式与启动时段。
统计只读取既有授权数据；[设计依据和统计口径](../docs/decisions/0018-admin-visual-analytics.md)
说明区间、时区、待收与实收、占用率和数据限制。
...
schema 3 的系统管理员可进入“管理员管理”，创建账号、修改显示名称、分配角色和站点范围、
启用或停用账号。站点管理员只查看和操作授权站点；用户管理员可查看用户与订单并冻结或解冻。
schema 3 所有管理员都可修改本人密码，成功后退出；旧 schema 1/2 隐藏账号管理并禁用改密，
其余原有业务不受影响。部署见 [003 说明](../database/README.md#启用管理员管理扩展-schema-3)。
客服工单在 schema 2/3 均保留，仅系统管理员可处理；每次读写检查当前角色、状态及首次改密。
