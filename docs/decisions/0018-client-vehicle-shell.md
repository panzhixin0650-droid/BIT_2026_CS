...
右侧面板内切换，避免手机式全屏覆盖。共享地图服务仍保留驾车、步行、公交、骑行能力，
但车载路线页只提供驾车，不显示其他方式选择；它仍只是路线规划和交互查看，不声称持续
GPS、语音播报、偏航检测、自动重算、实时交通保证或基于车辆剩余电量推荐。
...
- 登录页在车载尺寸下采用左右双栏，品牌说明位于左侧、登录卡位于右侧；手机端继续使用
  原纵向登录布局。路线规划成功后可隐藏应用顶栏、底栏和右侧面板，以横向全幅显示地图，
  左上控制条始终保留“退出全屏”按钮以避开右上指南针，并由独立切换按钮展开或收起
  服务端返回的分步路线。开始规划前，地图显示并聚焦所选充电站；成功后才切换到新路线
  画布，退出全屏后侧栏继续展示路线摘要和分步说明，不显示内部画布状态代替路线内容。
  这些交互不改变其“路线规划而非持续导航”的能力边界。
- 充电页与手机端共享圆环进度组件，并复用当前参考价、锁定价和预约超时提示；车载端按
  横屏重排为左侧中心进度、右侧指标与操作，不复制手机纵向页面。“查看我的订单”始终
  可见，即使没有当前订单也可进入历史订单列表。
- “我的”左栏居中突出本机头像及统一字号的昵称、手机号和余额；头像和昵称修改收进独立
  二级页，移除常驻的数据来源说明，服务提示位于修改入口下方，退出登录紧邻并固定在左栏
  底部；两个操作按钮提高到约 60 px，资料项使用均匀间距而不保留大块弹性空白。右栏下方
  只由订单、客服、报修、
  工单四个入口以 2×2 填满。订单页沿用手机端的状态徽标、摘要卡片与明细字段，并使用
  左侧列表、右侧详情的横向主从布局。
...
1. 登录成功，以及进入首页、充电或“我的”时主动刷新相关数据；程序恢复或网络重连后
   重新登录成功再刷新。没有服务端 EVENT，`CHARGING` 期间约每秒轮询 `order.current`，
   与手机端使用同一权威时间和自动结束状态；不把 Mock `order.progress` 的测试加速步长
   当作真实秒数显示。
...
4. 一端停止后，另一端下一次读取 `order.current`，必要时读取 `order.list` 找回最终
   `COMPLETED/PENDING_PAYMENT`。若并发中的 `order.progress` 收到 `40903`，同样转为刷新。
...
- 合入主线峰谷电价、预约超时和手机端视觉改进后，车载端复用了共享圆环、价格说明、
  预约有效期和头像选择能力；登录、充电和资料页仍保持独立横屏布局。
- 路线规划成功后进入横向地图全屏，左上控制条提供路线详情展开/收起和退出按钮；界面
  继续明确说明它不具备持续 GPS、语音播报、偏航检测和自动重算能力。
