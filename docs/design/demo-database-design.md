...
[ADR-0013 Demo 高峰加价](../decisions/0013-demo-peak-pricing.md) 不增加表或迁移：
...
计价只保留“一个站点一个基础价”。用户批准 ADR-0013 的固定 Demo 高峰 +20%，
...
| 动态峰谷/快慢充计价 | ADR-0013 仅启用固定高峰加价；动态配置另行设计 `pricing_rules` 与迁移，保留旧订单快照 |
