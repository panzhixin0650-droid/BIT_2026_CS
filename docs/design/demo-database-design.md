...
>
> 当前阶段：五表基线已冻结；ADR 0008 / 002 增加工单，ADR 0009 / 003 增加管理员授权与审计。

---

## 1. 结论

2026-09-07 增量说明：五表部分仍是 schema 1 基线；用户已明确启用
[ADR 0008 客服工单](../decisions/0008-support-ticket-desk.md)。
[002 迁移](../../database/migrations/002_support_tickets.sql) 在 schema 2 仅追加
`support_tickets`，保留原五表全部字段、ID 和订单快照，不启用其他旧版扩展。
工单属于用户，包含提交去重编号、标题、摘要、模型来源、状态、管理员回复和 UTC 时间；
唯一约束为 `(user_id, submission_id)`，外键关联 `users.user_id`。
创建和处理独立事务不触及订单、余额和电桩。状态、长度和权限详见
[工单 V1 契约](../../contracts/support-tickets-v1.md)。

[ADR-0010 扫一扫报修](../decisions/0010-scan-repair-tickets.md)进一步将设备报修并入工单。
[004 迁移](../../database/migrations/004_repair_tickets.sql)在 schema 3 上增加
`support_tickets.pile_code`（可空、外键引用 `charging_piles.pile_code`）及
`fault_type`（非空，普通客服单为空字符串，报修单 1–64 字符）。两字段由成对 CHECK
约束保护，按桩编号增加部分索引。外键禁止删除/改名已关联的桩；原有工单记录无设备关联。
工单的提交去重、状态、回复和用户权限不变，报修创建/处理不联动订单、余额或桩状态。
新服务端兼容 schema 1–4；只有报修写入需要 schema 4，升级必须停服备份后显式执行。

[ADR-0016 Demo 高峰加价](../decisions/0016-demo-peak-pricing.md) 不增加表或迁移：
站点保存基础价，开始时在应用层按固定高峰规则计算单价并写入现有订单快照；历史单不变。
...
| 用户确认的客服工单、回复和处理状态 | `support_tickets` |
...
1. `001` 建立五张核心表；`002` 增加工单；`003` 在 schema 2 上升级管理员并增加两张附属表。
...
| 通用请求幂等、全业务审计、迁移校验和 | 不实现；工单去重与管理员审计为已接受的局部扩展 | 有更多持久化消费者时 |

这些不是“永远禁止”；当前没有服务端持久化消费者的能力不建表，本地只读助理也不需要数据库迁移。
...
种子账号仍为 `admin / 123456`。旧 SHA-256 凭据用于兼容；完成 003 后成功登录才自动升级为 PBKDF2-SHA256；新建和改密直接使用 PBKDF2-SHA256。账号只停用、不删除，完整规则见 [ADR 0009](../decisions/0009-admin-account-rbac.md)。
...
| `longitude` | REAL | 是 | 经度 |
| `latitude` | REAL | 是 | 纬度 |
| `price_cents_per_kwh` | INTEGER | 是 | 当前站点统一基础价，单位分/kWh；不保存高峰参考价 |
| `status` | TEXT | 是 | `ACTIVE` 或 `DISABLED` |
| `created_at` | TEXT | 是 | 创建时间 |

计价只保留“一个站点一个基础价”。用户批准 ADR-0016 的固定 Demo 高峰 +20%，
在应用层按北京时间开始时刻计算并四舍五入到整数分，再写入订单单价快照。
预约不锁价，之后跨时段、改站点价格或补付都不会改变订单快照。
不建计价配置表，不做低谷折扣、服务费、停车费、快慢充倍率或跨时段分段计价。
...
- `migrations/002_support_tickets.sql`：追加工单，schema 1 → 2；
- `migrations/003_admin_accounts.sql`：在 schema 2 上升级管理员并增加授权、审计附属表，版本升为 3；
- `seeds/demo.sql`：生成课程页面所需的演示数据；
- `tests/`：在临时库中验证迁移、种子、订单事务、约束和失败分支；
- 可选样例数据库：只用于快速联调，不能替代 SQL。

种子时间以建库当天为基准，因此正式演示前应使用新的空数据库执行迁移和种子，确保“今日”和近 7/30 日曲线仍有数据。
...
| 动态峰谷/快慢充计价 | ADR-0016 仅启用固定高峰加价；动态配置另行设计 `pricing_rules` 与迁移，保留旧订单快照 |
