...
充电业务 TCP 连接。
腾讯模式保留登录窗口显示后的导航 SDK 异步预加载；预加载不请求路线、不发送当前位置，
失败时保持静默并在首次真实导航时重试。登录进入首页后，站点地图才创建可见画布，展示
`station.list` 返回的站点与当前选定位置。首页和导航共用 HTML 与加载器实现，各自保留
地图实例，导航的路线、缩放与跨 Tab 恢复不受首页刷新影响。
`--map mock` 不执行预加载、不创建 WebEngine，首页由 QPainter 绘制离线示意地图。
...
Marker）及 `station_preview_card.*`（站点信息）。Mock 地图中的道路、水域和区域块仅为
示意背景，Marker 使用接口经纬度投影；不请求真实底图、不伪造导航线路。Tencent 模式使用
