...
界面、样式与头像选择逻辑位于 `client/`，头像通过本地存储适配器保存；
视觉调整不改变业务 API、地图/智能助理服务、TCP 协议、数据库及服务端语义。
...
- 点击“更换头像”，可选择“小芽、小猫、小熊、小兔”四款基础头像之一，
  或点击“从本地选择图片”导入图片；选择后显示圆形预览，点击“使用此头像”才保存，
  取消或关闭不会改变原头像。无效图片显示提示并保留上一次有效选择；
- 四款头像由 QPainter 绘制，沿用森林绿与暖白主题，无需联网或额外图片插件。
  两种来源均归一化到最大 512×512、保存为 PNG，按用户隔离，重新登录或重启后恢复；
